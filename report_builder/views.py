...
from .models import Report, DisplayField, FilterField, Format, ReportDownload
from .utils import (
...
class DownloadXlsxView(DataExportMixin, View):
    ...
    def async_report_save(self, report, objects_list, title, header, widths):
        ...
        report_download.report_file.save(title, ContentFile(xlsx_file.getvalue()))
...
def check_status(request, pk, task_id):
    ...
    if res.state == 'SUCCESS':
        report = get_object_or_404(Report, pk=pk)
        link = report.get_latest_download().report_file.url
